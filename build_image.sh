...
echo "tag=$tag"
#time docker build -t mamba_gis . --no-cache 2>&1  | tee build.log
# time docker build -t mamba_gis . 2>&1  | tee build.log

docker tag mamba_gis docker-scipy-notebook_scipy_gis:$tag
